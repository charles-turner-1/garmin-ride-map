...
def clip_privacy(coords: list[Coord], center: Optional[Coord],
                 radius_m: float, jitter_m: float) -> list[Coord]:
    ...
    if not center or radius_m <= 0:
        return coords
    clon, clat = center
    kept = [(lon, lat) for lon, lat in coords
...
def load_existing() -> tuple[list[dict], set[str]]:
    ...
    if MANIFEST_PATH.exists():
        ...
        processed |= {pid for pid in manifest.get("processedIds", [])
...
def main() -> None:
    ...
    if not token:
        sys.exit("GARMIN_TOKENSTORE_B64 is not set. Run `python ingest/save_token.py` "
    ...
    for activity in activities:
        ...
        try:
            raw = client.download_activity(aid, dl_fmt=Garmin.ActivityDownloadFormat.GPX)
        ...
        processed.add(aid)  # mark processed even if empty (indoor/no-GPS) so we don't retry
        if len(coords) < 2:
            continue

        features.append(build_feature(activity, coords))
        new_count += 1
        print(f"  + {aid} {activity.get('activityType', {}).get('typeKey')} {len(coords)} pts")
